# AGENTS.md

じゃんけんの要素を戦闘に組み込んだ小規模RPG。TypeScript + Vite + Vitest。フレームワークは使わない。

このファイルは Codex 向けです。あなたには通常、**`docs/03_detailed-design.md` に書かれた
モジュールを1つ実装する**仕事が渡されます。設計は既に確定しているので、設計を作り直さないでください。

## 完了の定義

```bash
npm run check
```

内訳は型チェック（`tsc --noEmit`）、テスト（`vitest`）、レイヤ規約チェック
（`scripts/check-layers.sh`）の3つです。

### ただし、通すこと自体は目的ではありません

これが通ることは完了の**必要条件**であって、目的ではありません。
**目的は `docs/03_detailed-design.md` の仕様を満たすことで、テストはその確認手段です。**

次のような修正は、`npm run check` が通っても**不正解**です。

```ts
// 値を握りつぶしてテストを緑にしただけ。仕様を満たしていない
if (hp < 0) hp = 1;
```

- 特定の入力だけを場当たり的に分岐させる
- 期待値に合わせて計算式の定数を書き換える
- 例外を握りつぶす、`try/catch` で覆う
- 型を `any` にする、`as` で押し通す、`@ts-ignore` を付ける

**テストが通る間違った実装は、通らない実装より悪いです。** 製品に残り、
しかもテストがもう守ってくれません。

通し方が分からないときは、**無理に通さず、手を止めて報告してください。**
「どこまで実装し、何が分からないか」を書いてもらえれば、こちらで判断します。

## レイヤ規約（違反するとビルドが落ちます）

依存の向きは常に `ui → application → domain` の一方向。逆流させないでください。

| ディレクトリ | 責務 | 禁止事項 |
| --- | --- | --- |
| `src/domain/` | 純粋ロジック。状態を受け取り、新しい状態を返す | **`document` / `window` / `localStorage` / `Math.random()` / `Date.now()` / `new Date()` を書かない。`ui` `application` から import しない** |
| `src/application/` | ゲーム進行。domain を繋ぐ | `document` / `window` を触らない。`Math.random()` を書かない |
| `src/data/` | 敵・スキル・ステージの**数値定義のみ** | 関数・アロー関数・`if` / `for` / `switch` を書かない |
| `src/ui/` | 描画と入力 | ここ以外で DOM を触らない |
| `src/lib/` | 汎用ユーティリティ | ゲーム固有の知識を持ち込まない |

### 乱数について

**`Math.random()` を直接呼ばないでください。** `src/lib/rng.ts` の `Rng` を
**引数で受け取る**形にします。同じシードで同じ試合が再現できる状態を壊さないためです。

```ts
// 正しい
export function chooseHand(enemy: Enemy, rng: Rng): Hand { ... }

// 間違い
export function chooseHand(enemy: Enemy): Hand {
  return HANDS[Math.floor(Math.random() * 3)];
}
```

### 数値の置き場所

後で調整したくなる数値（敵のHP、攻撃力、スキルの倍率）は `src/data/` に置き、
ロジック側にハードコードしないでください。構造上の定数（じゃんけんの手が3種類）は例外です。

## テストは既に書かれています

`tests/` のテストは、実装より先に、設計だけを根拠に書かれています。
**あなたの仕事は、そのテストを通す実装を書くことです。**

**`tests/` 配下を変更しないでください。** テストが落ちるのは実装が仕様を満たしていない
ということです。テストを緩めて通すのは、この工程の目的を壊します。

テストの期待値そのものが設計と矛盾していると判断した場合は、テストを直さず、
**手を止めて「どのテストが、設計のどこと、どう矛盾しているか」を報告してください。**

## 修正だけを頼まれたとき

「`tests/scenario/battle.test.ts` が失敗している。`src/domain/battle.ts` だけを直して」
のように、**触れてよいファイルを指定される**ことがあります。

その場合、**指定されたファイル以外を変更しないでください。** 範囲を広げると、
どの変更が効いたのか分からなくなり、別の箇所を壊しても気づけません。

指定されたファイル以外にも原因があると判断した場合は、**直さずに報告してください。**
「どのファイルの何が原因だと考えるか」を書いてもらえれば、こちらで範囲を広げます。

同時に「あるべき挙動」も渡されます。**テストではなく、そちらを満たしてください。**

## 設計と食い違うとき

実装してみて設計が破綻していると分かった場合、**勝手に設計を変えないでください。**
実装を止めて、「設計のどこが、なぜ成立しないか」を報告してください。判断はこちらで行います。

## 触らないもの

- **`tests/` 配下**（実装より先に確定している仕様）
- `docs/` 配下（設計ドキュメント）
- `.claude/` 配下
- `CLAUDE.md`, `AGENTS.md`
- `scripts/check-layers.sh`

規約に合わないコードを通すために検査スクリプトを緩めることは、**しないでください。**

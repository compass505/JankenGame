...
| --- | --- | --- | --- | --- | --- |
| 突破率（貪欲） | 88.6% | 81.3% | 84.2% | 74.4% | **62.5%** |
...
通しクリア率 28.2% / 1戦 12.9ターン / 機械パターン最良 7.3%（差 **20.9pt**）。
...
| 敵 | グー / チョキ / パー | 主な手 | 隔たり |
| --- | --- | --- | --- |
| 1 かかし | 25.5 / 57.3 / 17.3 | チョキ | 11.5% |
| 2 はさみ鳥 | 30.2 / 67.3 / **2.5** | チョキ | 23.7% |
| 3 岩の番人 | 41.6 / 15.3 / 43.0 | パー | **30.5%** |
| 4 紙の使者 | 11.4 / 49.1 / 39.5 | チョキ | 16.6% |
| 5 グリコ王 | **32.8 / 31.3 / 35.9** | パー | 14.4% |
| | | | 平均 19.3% |

> **「隔たり」だけを目標にしてはいけない**（2026-08-11 の `/balance` で分かった）。
> これは**5体の平均との差**なので、**どの戦闘も3手を均等に使う状態にすると 0 に近づく。**
> 隔たりを最大化することは、**敵ごとに違う一択を作ること**とほぼ同義になる。
>
> **一択度は別に見る。その戦闘で最も使われた手の割合**を並べるとよい。
> 値表を入れる前は 57.3 / 72.1 / 43.0 / 48.8 / 59.2（平均 **56.1%**）、
> 入れた後は 57.3 / 67.3 / 43.0 / 49.1 / **35.9**（平均 **50.5%**）。
> **グリコ王が 59.2% → 35.9% とほぼ三等分になったのが最大の成果**であり、
> 隔たりが 29.1% → 14.4% に下がったのは**均等に近づいた結果**であって悪化ではない。
...
### 通算の手の分布がチョキに寄っていることについて（2026-08-11・**放置と決めた**）

通算は **28.0 / 45.8 / 26.3**。ADR 0003 が揃えた 29.8 / 38.7 / 31.5 から開いている。
**慌てて直さないこと。** 内訳を見ると、性質の違う3つが混ざっている。

| 敵 | チョキ | 中身 |
| --- | --- | --- |
| 1 かかし | 57.3% | 偏りが無い＝**読む材料が無い**。素の打点が高い手が正解になる |
| 2 はさみ鳥 | 67.3% | **計測の盲点**（下記）。実際の人間はここまで寄らないはず |
| 3 岩の番人 | 15.3% | グー偏重なのでチョキは負け筋 |
| 4 紙の使者 | 49.1% | 耐性 ×0.5 で弱いが、パー偏重の敵には勝ち筋 |
| 5 グリコ王 | 31.3% | 三等分。健全 |

**放置してよい理由。**

1. **ADR 0003 が守りたかった性質は満たされている。** あのADRが問題にしたのは
   「**グーが 10% しか使われず、状態を見ないプレイと見るプレイの勝率が同じ**」という状態
   （`docs/01_requirements.md` 冒頭）。いまは グー 28.0 / パー 26.3 で壊れた 10% 圏から遠く、
   機械パターンとの差も 20.9pt ある。**38.7% は目標ではなくスナップショットだった**
2. **かかしの 57.3% は ADR 0001 の設計どおり。** 3手はそれぞれ「最良になる局面」を持ち、
   **チョキの局面は「情報が無く、HPが満タンで、にらみが 0」**。それは1戦目そのものである
3. **はさみ鳥の 67.3% は計測の盲点。** 貪欲プレイは1手先読みなので、
   **あいこでにらみを溜めると敵のグーが化ける**（被弾 3 → 10 → 17）ことを構造的に見られない

**直すとしたら、順番がある。**

**先に計測を直すこと。** `scoreTurn` はにらみを**プレイヤー側の `stareBonus`** でしか評価していない。
敵側の倍率も引くようにしてから測り直し、**それでも残るぶんだけ**を扱う。
盲点のある物差しで `src/data/` を動かさない。

**game の値で直そうとした場合のコストも見ておくこと。**
`BASE_HANDS` のチョキを下げると全5戦のカーブを取り直しになる。
かかしにチョキ耐性を付けると一択は消えるが、**「くせがない・耐性なし」という
練習台の役割を壊す**（`docs/00_research.md` 節9.5「1体目は標準・耐性なし」）。
どちらも、得られるのは「集計値が揃って見える」ことだけ。
...
**1体に振るのは1項目だけ**（`docs/00_research.md` 節9.5「3軸を同時に投入しない」）。

| 敵 | 使う項目 | 性格 | 画面に出る文 |
| --- | --- | --- | --- |
| かかし | **なし** | 練習台。基準の値表を教える | — |
| はさみ鳥 | `stareBonus` 7 | **溜めると危険** | `グーはにらみ1つにつき +7` |
| 岩の番人 | **なし** | 偏り .60 が既に強く、値表を足すと一択が深まった | — |
| 紙の使者 | `damage`（パー2） | **攻撃が軽い**。負けの代償が小さくグーを出せる | （被弾の列に出る） |
| グリコ王 | `damage` 7 ＋ `heal` 0 | **回復しない王** | `パーで回復しない` |

> **敵のダメージを上げても一択は壊れない。** `/balance` で一度そう振って外した。
> 罰を強くすると**それに負ける手が選択肢から消えるだけ**で、天秤にはならない。
> 実際、紙の使者のチョキを 8 にしたらチョキが 48.8% → 62.6% に**増えた**（残る手が減ったため）。
> **攻撃を軽くしてグーを出せるようにしたほうが、手が散った。**

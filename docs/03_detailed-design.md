...
| 3 | `shearBird` | はさみ鳥 | 26 | .15 / **.55** / .30 | .20 / .45 / .35 | なし | **+2** | standard |
...
| | 1 かかし | 2 岩の番人 | 3 はさみ鳥 | 4 紙の使者 | 5 グリコ王 |
| --- | --- | --- | --- | --- | --- |
| 突破率（貪欲） | 89.0% | 83.3% | 85.7% | 70.9% | **66.4%** |

**チョキ好みの敵は構造的に安全**なので、放っておくと3体目が谷になる。
返しがグーで、外したときに返ってくるのが**最も軽いパー**だからで、
逆にパー好みの敵（返しがチョキ、外すとにらみの乗ったグーが返る）が最も重い。
はさみ鳥はこれを **HP・パーの比率・本気強化 +2** で埋めてある。

通しクリア率 29.9% / 1戦 12.9ターン / 機械パターン最良 6.6%（差 23.4pt）。

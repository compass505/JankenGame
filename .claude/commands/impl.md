...
**軸は「残っている仕事が読むことか、決めることか」。** 読むことは委譲でき、決めることはできない。

**第1段階 — 失敗が原因箇所を示しているか**

| 失敗の出かた | 対応 |
| --- | --- |
| 型エラー・レイヤ規約違反 | **luna に直接差し戻す。**出力をそのまま渡す |
| 例外・スタックトレースが実装を指している | **luna に直接差し戻す。**出力をそのまま渡す |
| アサーション失敗・ハング | 自分で失敗を読む → 第2段階へ |

**第2段階 — 読んだうえで、残りは何か**

| 状態 | 対応 |
| --- | --- |
| **原因も直し方も分かった** | **自分で直す。**渡さない。残りが数行なら往復のほうが高い |
| **見当はつくが、確定にはコードを読む必要がある** | **luna に渡す**（下記の形式で） |
| **見当もつかない** | **`docs/03` を疑う。**設計の穴ならユーザーに報告してから直す |

どちらでも2回外れたら、そこで初めて腰を据えて診断する。

### luna に渡すときの形式

**「触れてよいファイル」と「あるべき挙動」を必ず両方書く。**
...
- **ファイルを絞らない** → 目についた箇所を場当たり的に直し、別の層を壊す
- **あるべき挙動を書かない** → `if (hp < 0) hp = 1` のように、
  テストを緑にすることだけを目的とした修正が返る。**これは遅い修正より悪い**

渡す前にコミットしてあれば外れても `git checkout .` で戻せるが、
**戻すのが無料でも、外れた往復のトークンは戻らない。** 雑に渡さない。
